...
function host(): AndroidHost & { calls: string[] } {
  ...
  return { calls, loadSnapshot: async () => snapshot, saveSettings: async () => {}, clearSettings: async () => { snapshot = null; calls.push('clearSettings'); }, appendHistory: async () => {}, replaceHistory: async (historyJson) => { snapshot = snapshot && { ...snapshot, historyJson }; calls.push('replaceHistory'); }, clearHistory: async () => { snapshot = snapshot && { ...snapshot, historyJson: [] }; calls.push('clearHistory'); }, savePet: async () => {}, deletePet: async () => {}, clearPets: async () => { snapshot = snapshot && { ...snapshot, pets: [] }; calls.push('clearPets'); }, selectPet: async () => {}, subscribe: () => () => undefined };
}
